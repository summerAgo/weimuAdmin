...
layui.define(['jquery','layer','element','form'], function(exports) {
	...
	DTree.prototype.loadListTree = function(pListData, listData, level, $ul){		
		...
		if (pListData.length > 0){
			for (var i = 0; i < pListData.length; i++) {
				...
				var checkArr = parseData.checkArr(), isLast = parseData.isLast();
				if(checkArr && checkArr.length > 0){
					_this.checkArrLen = checkArr.length;		// 获取复选框个数
				}
				
				// 2. 根据已知数据的id判断该条数据是否还有子数据
				var childListData = _this.queryListTreeByPid(parseData.treeId(), listData);
				if(childListData.length == 0){ isLast = parseData.isLastT();} // 不存在,则为最后一级节点  
				
				// 3. 页面元素加载数据
				if ($ul.hasClass(UL_ROOT)) {
					var dom = _this.getDom(parseData.treeId(), parseData.parentId(), parseData.title(), parseData.isLast(), parseData.iconClass(), parseData.checkArr(), parseData.level());
					$ul.append(_this.getLiItemDom(dom, parseData.treeId(), parseData.parentId(), parseData.isLast(), parseData.level(), parseData.basicData(), "root"));
				} else {
					var dom = _this.getDom(parseData.treeId(), parseData.parentId(), parseData.title(), parseData.isLast(), parseData.iconClass(), parseData.checkArr(), parseData.level());
					$ul.append(_this.getLiItemDom(dom, parseData.treeId(), parseData.parentId(), parseData.isLast(), parseData.level(), parseData.basicData(), "item"));
				}
	...
	DTree.prototype.loadTree = function(root, $ul){
		...
		if (root) {
			...
			for (var i = 0; i < root.length; i++) {	// 遍历跟节点或追加的跟节点
				...
				if(typeof data !== "object") continue;
				var parseData = _this.parseData(data);
				var checkArr = parseData.checkArr();
				...
				if (children.length != 0) {
					var treeId = parseData.treeId();
					_this.loadChildTree(children, _this.obj.find("ul[data-id='"+parseData.treeId()+"']"));
				}
			}
		}
	};
	...
	DTree.prototype.loadChildTree = function(children, $ul){	
		...
		for (var i = 0; i < children.length; i++) {	// 遍历子节点
			...
			var child = parseData.children();
			if (child.length != 0) {
				_this.loadChildTree(child, _this.obj.find("ul[data-id='"+parseData.treeId()+"']"));
			}
		}
	};
	...
	DTree.prototype.parseData = function(data) {	
		...
		return {
			...
			isLast: function(){
				return (typeof (data[_this.response.isLast]) === "boolean") ? data[_this.response.isLast] : false;
			},
			isLastT: function(){
				return (typeof (data[_this.response.isLast]) === "boolean") ? data[_this.response.isLast] : true;
			},
			checkArr: function(){
				var checkArr = [];
		...
		return {
			...
			checkbox: function() {	// 复选框
				...
				if(flag){
					...
					if(checkArr && checkArr.length > 0){	// 如果此处报错，说明后台数据中没有提供checkArr字段
						...
						for(var j=0; j<addNodeBarContents.length; j++){
							...
							switch (type) {
								...
								case "textarea":
									...
									break;
								case "select":
									var optionsData = addNodeBarContents[j].optionsData;
									var options = "";
